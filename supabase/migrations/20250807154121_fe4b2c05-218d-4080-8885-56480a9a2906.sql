...
-- Corrected with proper enum values
...
-- Step 3: Convert time_mode to enum (using correct enum values)
ALTER TABLE initial_tasks ALTER COLUMN time_mode TYPE time_mode USING 
  CASE 
    WHEN time_mode = 'start-end' THEN 'start-end'::time_mode
    WHEN time_mode = 'duration' OR time_mode = 'start-duration' THEN 'start-duration'::time_mode
